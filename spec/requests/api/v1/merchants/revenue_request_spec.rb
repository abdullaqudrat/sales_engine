...
describe "Merchants API" do
  ...
  it "can get total revenue of merchants by date"  do
    ...
    expect(response).to be_successful
    expect(total_revenue_by_date_1).to eq({"total_revenue" => "20000"})
    expect(total_revenue_by_date_2).to eq({"total_revenue" => "11500"})
  end
  it "can get total revenue of merchant"  do
    ...
    expect(response).to be_successful
    expect(total_revenue).to eq({"revenue"=>"31500"})
  end
  it "can get total revenue of merchants by date"  do
    merchant = create(:merchant)
    c_1, c_2, c_3, c_4, c_5 = create_list(:customer, 5)
    ...
    expect(response).to be_successful
    expect(total_revenue_by_date_1).to eq({"revenue"=>"20000"})
    expect(total_revenue_by_date_2).to eq({"revenue"=>"11500"})
  end
end
